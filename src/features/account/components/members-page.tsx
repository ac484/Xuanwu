...
export function MembersPage() {
  ...
  const handleRecruitMember = async () => {
    ...
    try {
      ...
      toast({
        title: t("account.identityResonanceActivated"),
        description: t("account.identityResonanceDescription"),
      });
    } catch (error: unknown) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map((member) => (
          <Card
            key={member.id}
            className="border-border/60 bg-card/50 backdrop-blur-sm hover:shadow-md transition-all group overflow-hidden"
          >
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-4 mt-2">
                <Badge
                  variant="outline"
                  className="text-[9px] uppercase font-bold bg-primary/5 px-2 py-0.5 border-primary/20 text-primary"
                >
                  {member.role}
                </Badge>
