...
A key design principle in this application is the use of polymorphic view components. This pattern avoids code duplication when a feature needs to be presented in different contexts (e.g., at a single workspace level vs. an aggregated organization level).
...
- **Implementation**:
    ...
    - A **workspace-specific** container (e.g., `WorkspaceAuditPage`) fetches **filtered, local data** from its context (`useWorkspace`) and passes it to the *same* view.
